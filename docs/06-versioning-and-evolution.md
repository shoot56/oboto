...
- **Theme version**: maintained in `style.css` (currently `Version: 1.2.25.2`).
...
- **MCP Server rewrites**
  ...
  - When the synchronized payload persistence, duplicate-reconciliation, or stale-record cleanup model changes, bump `MCP_Server_Sync::DATA_VERSION`; the cached catalog will repair generated posts and post meta once after deployment.
