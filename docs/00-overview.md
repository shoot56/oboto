...
- Every catalog entry is synchronized into a read-only `mcp-server` post and rendered at `/mcp-catalog/<yaml-filename>/`, regardless of the host used by `repoURL`. The directory-qualified source path is retained for cache identity and GitHub source links while public slugs remain based on the YAML filename.
